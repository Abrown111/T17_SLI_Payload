...
from struct import unpack, calcsize, unpack_from, pack_into
...
class Struct:
    ...
    def __init__(self, register_address, struct_format):
        ...
        self.buffer = bytearray(1 + calcsize(self.format))
        self.buffer[0] = register_address
        self.register = register_address

    def __get__(self, obj, objtype=None):
        self.buffer[0] = self.register
        obj.i2c.writeto(obj.device_address, self.buffer)
        obj.i2c.readfrom_into(obj.device_address, self.buffer)
        return unpack_from(self.format, memoryview(self.buffer)[1:])
